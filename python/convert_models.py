...
def _decompose_fused_depthwise(graph_def):
    ...
    for node in graph_def.node:
        ...
        fused_ops_raw = list(node.attr['fused_ops'].list.s) if 'fused_ops' in node.attr else []
        fused_ops = [s.decode('utf-8') if isinstance(s, bytes) else s for s in fused_ops_raw]

        # Get conv attributes
        strides = list(node.attr['strides'].list.i) if 'strides' in node.attr else [1, 1, 1, 1]
        padding = node.attr['padding'].s if 'padding' in node.attr else b'SAME'
        dilations = list(node.attr['dilations'].list.i) if 'dilations' in node.attr else [1, 1, 1, 1]
        data_format = node.attr['data_format'].s if 'data_format' in node.attr else b'NHWC'
...
def convert_model(models_dir, model_name):
    ...
    print(f'  Patched {n_weights} weights, decomposed {n_decomposed} fused DW ops, {n_attrs} TArgs attrs',
          file=sys.stderr)
    ...
    with tf.compat.v1.Graph().as_default() as graph:
        ...
        if not output_nodes:
            ...
            for op in all_ops:
                if (op.name not in consumed and
                        op.type not in ('Const', 'NoOp', 'Placeholder', 'Identity')):
                    output_nodes[op.name] = op.outputs[0]

        if not input_nodes or not output_nodes:
            raise RuntimeError(f'Could not determine input/output nodes for {model_name}')
        ...
        with tf.compat.v1.Session(graph=graph) as sess:
            ...
            for key, tensor in input_nodes.items():
                sig_inputs[key] = tf.compat.v1.saved_model.utils.build_tensor_info(tensor)

            sig_outputs = {}
            for key, tensor in output_nodes.items():
                sig_outputs[key] = tf.compat.v1.saved_model.utils.build_tensor_info(tensor)
