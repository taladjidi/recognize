...
def run_benchmark():
    ...
    for i, path in enumerate(paths):
        ...
        if is_warmup:
            print_err(
                f"  Warm-up: pre={t_pre.elapsed * 1000:.1f}ms inf={t_inf.elapsed * 1000:.1f}ms post={t_post.elapsed * 1000:.1f}ms faces={len(faces)}"
            )
        elif i == 1:
            print_err(
                f"  Steady:  pre={t_pre.elapsed * 1000:.1f}ms inf={t_inf.elapsed * 1000:.1f}ms post={t_post.elapsed * 1000:.1f}ms faces={len(faces)}"
            )
