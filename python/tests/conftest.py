...
def has_imagenet_model():
    """Check if EfficientNetV2 SavedModel is available."""
    path = os.path.join(MODELS_DIR, 'efficientnetv2_saved', 'saved_model.pb')
    if not os.path.isfile(path):
        pytest.skip('Model not converted: efficientnetv2_saved (run convert_models.py)')
    return True
...
def has_landmark_models():
    ...
    for region in regions:
        ...
        if not os.path.isfile(path):
            pytest.skip(f'Model not converted: landmarks_{region}_saved (run convert_models.py)')
    return True
