    ...
    def test_eiffel_tower(self, run_classifier, has_landmark_models, eiffeltower_jpg):
        ...
        if len(result) >= 2:
            assert result[0] == "landmark", (
                f'First element should be "landmark", got {result[0]}'
            )
            assert "eiffel" in result[1].lower(), (
...
class TestGeo:
    ...
    def test_no_gps(self, run_classifier, alpine_jpg):
        ...
        print(f"  Result: {result}")
        assert result == [], (
            f"Expected empty list for non-geotagged image, got {result}"
        )
    ...
    def test_geo_batch(self, geotagged_jpg, geotagged_germany_jpg):
        ...
        for i, parsed in enumerate(json_lines):
            assert isinstance(parsed, list), (
                f"Result {i}: expected list, got {type(parsed)}"
            )
