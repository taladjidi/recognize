...
from classifier_landmarks import (
    REGIONS, MODELS_DIR, IMG_SIZE, TOP_K, THRESHOLD, BATCH_SIZE,
    load_labels, preprocess_image, get_top_k, _load_models_v1,
)
...
def run_benchmark():
    ...
    for batch_start in range(0, len(paths), BATCH_SIZE):
        ...
        if is_warmup:
            region_summary = ' '.join(f'{r.split("_")[-1][:3]}={t*1000:.0f}ms' for r, t in region_times.items())
            print_err(f'  Warm-up batch ({n}): pre={t_pre.elapsed*1000:.0f}ms inf={t_inf.elapsed*1000:.0f}ms [{region_summary}]')
        else:
