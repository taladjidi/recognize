...
from classifier_musicnn import (
    MODELS_DIR, TOP_K, BATCH_FRAMES, MSD_CLASSES, rules,
    get_ffmpeg_binary, transcode_audio, compute_mel_spectrogram,
)
import rules_engine
...
def run_benchmark():
    ...
    for i, path in enumerate(paths):
        ...
        with TimingContext('preprocess') as t_pre:
            ...
            batches = tf.stack([mel_spec[j * BATCH_FRAMES:(j + 1) * BATCH_FRAMES] for j in range(num_batches)])
        ...
        with TimingContext('postprocess') as t_post:
            ...
            results = [{'className': MSD_CLASSES[idx], 'probability': float(probabilities[idx])} for idx in indices]
            labels = rules_engine.apply_rules(results, rules, uppercase=False)

        gpu = gpu_snapshot()

        total = t_ffmpeg.elapsed + t_mel.elapsed + t_pre.elapsed + t_inf.elapsed + t_post.elapsed
        ...
        if is_warmup:
            print_err(f'  Warm-up: ffmpeg={t_ffmpeg.elapsed*1000:.0f}ms mel={t_mel.elapsed*1000:.0f}ms '
                      f'inf={t_inf.elapsed*1000:.0f}ms ({num_batches} batches) → {labels}')
        elif i == 1:
            print_err(f'  Steady:  ffmpeg={t_ffmpeg.elapsed*1000:.0f}ms mel={t_mel.elapsed*1000:.0f}ms '
