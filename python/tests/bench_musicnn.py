"""Benchmark: Audio classifier (YAMNet + MusicNN, ONNX).

Measures model load, warm-up, transcode, inference.
Run: python tests/bench_musicnn.py [--no-gpu]
"""
...
TEST_AUDIO = os.path.join(TEST_RES, "Rock_Rejam.mp3")


def bench_musicnn(n_repeats=3, gpu=True):
    from classifiers.musicnn import AudioClassifier

    report = BenchmarkReport("Audio (YAMNet + MusicNN ONNX)")

    # Model load
    with TimingContext("model_load") as t_load:
        clf = AudioClassifier(MODELS_DIR, gpu=gpu)
    ...
    for i in range(n_repeats):
        with TimingContext("total") as t_total:
            with TimingContext("classify") as t_cls:
                labels = clf.classify(TEST_AUDIO)

        gpu_snap = gpu_snapshot()
        report.add(BenchmarkResult(
            classifier="musicnn",
            file_path=os.path.basename(TEST_AUDIO),
