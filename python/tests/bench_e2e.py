...
CLASSIFIERS = {
    'imagenet': {
        ...
        'files': ['alpine.JPG', 'eiffeltower.jpg', 'casey-lee-awj7sRviVXo-unsplash.jpg'],
        'repeat': 7,  # ~21 files
    },
    'landmarks': {
...
def build_file_list(config):
    ...
    if config['files']:
        base = [os.path.join(RES_DIR, f) for f in config['files']
...
def benchmark_classifier(name, config):
    ...
    print(f'  [{name}] Done: {total_elapsed:.1f}s total, first={time_to_first:.1f}s, '
          f'steady={avg_steady*1000:.0f}ms/file', file=sys.stderr)
...
def main():
    ...
    print(f'{"=" * 50}', file=sys.stderr)
    print(f'  {"Classifier":12s} {"First":>8s} {"Steady":>10s} {"Through":>10s}', file=sys.stderr)
    print(f'  {"":12s} {"(s)":>8s} {"(ms/file)":>10s} {"(files/s)":>10s}', file=sys.stderr)
    for name, data in combined['classifiers'].items():
        print(f'  {name:12s} {data["time_to_first_s"]:8.1f} {data["avg_steady_ms"]:10.0f} '
              f'{data["throughput_files_per_s"]:10.1f}', file=sys.stderr)
