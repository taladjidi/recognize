...
def build_and_export():
    ...
    print(
        f"Warmup inference: {t_warmup:.3f}s, output shape: {out.shape}", file=sys.stderr
    )
    ...
    n_fused_dw = sum(
        1 for n in frozen_graph.node if n.op == "_FusedDepthwiseConv2dNative"
    )
    n_decomposed_dw = sum(
        1 for n in frozen_graph.node if n.op == "DepthwiseConv2dNative"
    )
    ...
    with tf.compat.v1.Session(
        graph=g, config=tf.compat.v1.ConfigProto(allow_soft_placement=True)
    ) as sess:
